...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let wrongs = safeJSONParse("ke_wrongs", []);
  let masteredIds = safeJSONParse("ke_mastered", []); // 3回以上正解した問題
  let correctCounts = safeJSONParse("ke_correct_counts", {}); // 各問題の正解回数
  let currentStreak = parseInt(localStorage.getItem("ke_streak")) || 0; // 連続正解数
  let totalAnswered = parseInt(localStorage.getItem("ke_total_answered")) || 0; // 総解答数
  let totalCorrect = parseInt(localStorage.getItem("ke_total_correct")) || 0; // 総正解数
  ...
  const btnPast = document.getElementById("card-mode-past");
  const btnMaterials = document.getElementById("card-mode-materials");
  ...
  function updateHomeStats() {
    ...
    if (typeof quizData !== 'undefined') {
      totalQCount = (quizData.quiz?.length || 0) + (quizData.past_exams?.length || 0) + (quizData.materials?.length || 0);
    }
  ...
  function startMode(mode, reviewFilter = "all") {
    ...
    if (mode === "quiz") {
      ...
      modeNameIndicator.textContent = "小テストモード";
    } else if (mode === "past") {
      questionsSrc = quizData.past_exams || [];
      modeNameIndicator.textContent = "過去問モード";
    } else if (mode === "materials") {
      questionsSrc = quizData.materials || [];
      modeNameIndicator.textContent = "授業資料問題モード";
    }
  ...
  function checkAnswer() {
    ...
    // 総解答数・正解数の更新
    totalAnswered++;
    localStorage.setItem("ke_total_answered", totalAnswered);
    if (isCorrect) {
      totalCorrect++;
      currentStreak++;
      localStorage.setItem("ke_total_correct", totalCorrect);
      localStorage.setItem("ke_streak", currentStreak);
      // 正解回数の累計更新 → 3回以上で習得済みに
      correctCounts[q.id] = (correctCounts[q.id] || 0) + 1;
      localStorage.setItem("ke_correct_counts", JSON.stringify(correctCounts));
      if (correctCounts[q.id] >= 3 && !masteredIds.includes(q.id)) {
        masteredIds.push(q.id);
        localStorage.setItem("ke_mastered", JSON.stringify(masteredIds));
      }
    } else {
      currentStreak = 0;
      localStorage.setItem("ke_streak", currentStreak);
    ...
    if (isCorrect) {
      if (wrongs.includes(q.id)) {
        ...
        localStorage.setItem("ke_wrongs", JSON.stringify(wrongs));
      }
    } else {
      if (!wrongs.includes(q.id)) {
        wrongs.push(q.id);
  ...
  if (btnQuiz) {
    ...
    const innerQuizBtn = btnQuiz.querySelector("button");
    if (innerQuizBtn) innerQuizBtn.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); startMode("quiz"); });
  }

  if (btnPast) {
    btnPast.addEventListener("click", (e) => { e.preventDefault(); startMode("past"); });
    const innerPastBtn = btnPast.querySelector("button");
    if (innerPastBtn) innerPastBtn.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); startMode("past"); });
  }
  
  if (btnMaterials) {
    btnMaterials.addEventListener("click", (e) => { e.preventDefault(); startMode("materials"); });
    const innerMaterialsBtn = btnMaterials.querySelector("button");
    if (innerMaterialsBtn) innerMaterialsBtn.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); startMode("materials"); });
  }
